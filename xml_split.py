...
def start_element(name, attrs):
    ...
    if start is not None:
        ...
        cur_file.write(u'<%s%s>' % (start[0], attrs_s(start[1])))
    start = (name, attrs)
    if ending:
        return
...
def main(filename, output_dir):
    ...
    cur_file = open(os.path.join(out_dir, root + FMT % cur_idx + ext), 'w', encoding='utf-8')

    with open(os.path.join(out_dir, filename), 'r', encoding='ascii') as xml_file:
        while True:
            # Read a chunk
